...
public class LineChart: Chart {
    ...
    public typealias ChartLine = (chartPoints: [(Double, Double)], colors: [UIColor])
    ...
    public init(frame: CGRect, chartConfig: ChartConfigXY, xTitle: String, yTitle: String, lines: [ChartLine]) {
        ...
        let lineLayers: [ChartLayer] = lines.map {
            line in let chartPoints = line.chartPoints.map {chartPointScalar in
                ChartPoint(x: ChartAxisValueDouble(chartPointScalar.0), y: ChartAxisValueDouble(chartPointScalar.1))
            }
            
            let lineModel = ChartLineModel(chartPoints: chartPoints, lineColors: line.colors, positions: [0], animDuration: 0.5, animDelay: 0)
