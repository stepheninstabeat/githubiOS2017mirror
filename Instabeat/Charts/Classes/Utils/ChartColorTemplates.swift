...
public class ChartColorTemplates: NSObject
{
    ...
    {
        let leftParenCharset: NSCharacterSet = NSCharacterSet(charactersIn: "( ")
        let commaCharset: NSCharacterSet = NSCharacterSet(charactersIn: ", ")
        ...
        if colorString.hasPrefix("#")
        {
            ...
            index = colorString.index(colorString.startIndex, offsetBy: 1)
            length = length - 1
        ...
        else if colorString.hasPrefix("rgba")
        {
            ...
            scanner.scanCharacters(from: leftParenCharset as CharacterSet, into: nil)
            scanner.scanInt32(&r)
            scanner.scanCharacters(from: commaCharset as CharacterSet, into: nil)
            scanner.scanInt32(&g)
            scanner.scanCharacters(from: commaCharset as CharacterSet, into: nil)
            scanner.scanInt32(&b)
            scanner.scanCharacters(from: commaCharset as CharacterSet, into: nil)
