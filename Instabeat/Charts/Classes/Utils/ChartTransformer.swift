...
public class ChartTransformer: NSObject
{
    ...
    public func getTransformedValueBarChart(entry: ChartDataEntry, xIndex: Int, dataSetIndex: Int, phaseY: CGFloat, dataSetCount: Int, groupSpace: CGFloat) -> CGPoint
    {
        ...
        pointValueToPixel(point: &valuePoint)
        
        return valuePoint
    }
    
    /// Transforms an Entry into a transformed point for horizontal bar chart
    public func getTransformedValueHorizontalBarChart(entry: ChartDataEntry, xIndex: Int, dataSetIndex: Int, phaseY: CGFloat, dataSetCount: Int, groupSpace: CGFloat) -> CGPoint
    {
        // calculate the x-position, depending on datasetcount
        let x = CGFloat(xIndex + (xIndex * (dataSetCount - 1)) + dataSetIndex) + groupSpace * CGFloat(xIndex) + groupSpace / 2.0
        let y = entry.value
    ...
    public func pointValuesToPixel( pts: inout [CGPoint])
    {
    ...
    public func pointValueToPixel( point: inout CGPoint)
    {
        point = point.applying(valueToPixelMatrix)
    }
    
    /// Transform a rectangle with all matrices.
    public func rectValueToPixel( r: inout CGRect)
    {
        r = r.applying(valueToPixelMatrix)
    }
    
    /// Transform a rectangle with all matrices with potential animation phases.
    public func rectValueToPixel( r: inout CGRect, phaseY: CGFloat)
    {
    ...
    /// Transform a rectangle with all matrices.
    public func rectValueToPixelHorizontal( r: inout CGRect)
    {
        r = r.applying(valueToPixelMatrix)
    }
    
    /// Transform a rectangle with all matrices with potential animation phases.
    public func rectValueToPixelHorizontal( r: inout CGRect, phaseY: CGFloat)
    {
        // multiply the height of the rect with the phase
    ...
    public func rectValuesToPixel( rects: inout [CGRect])
    {
        let trans = valueToPixelMatrix
    ...
    public func pixelsToValue( pixels: inout [CGPoint])
    {
    ...
    public func pixelToValue( pixel: inout CGPoint)
    {
    ...
    {
        return
            _matrixValueToPx.concatenating(
