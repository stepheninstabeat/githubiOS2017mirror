...
public class PieRadarChartViewBase: ChartViewBase
{
    ...
    internal override func initialize()
    {
        ...
        #if !os(tvOS)
            _rotationGestureRecognizer = NSUIRotationGestureRecognizer(target: self, action: #selector(rotationGestureRecognized(recognizer:)))
    ...
    public var rotationAngle: CGFloat
    {
        ...
        set
        {
            ...
            _rotationAngle = ChartUtils.normalizedAngleFromAngle(angle: newValue)
            setNeedsDisplay()
        }
    }
    ...
    public func getSelectionDetailsAtIndex(xIndex: Int) -> [ChartSelectionDetail]
    {
        ...
        {
            guard let dataSet = data.getDataSetByIndex(index: i) else { continue }
            ...
            let yVal = dataSet.yValForXIndex(x: xIndex)
            if (yVal.isNaN)
            {
                continue
            }
    ...
    {
        spin(duration: duration, fromAngle: fromAngle, toAngle: toAngle, easing: easingFunctionFromOption(easing: easingOption))
    }
    ...
    @objc private func tapGestureRecognized(recognizer: NSUITapGestureRecognizer)
    {
        if (recognizer.state == NSUIGestureRecognizerState.ended)
        {
            ...
            if (distance > self.radius)
            {
                ...
                self.highlightValues(highs: nil)
                
                if _lastHighlight == nil
                {
                    self.highlightValues(highs: nil) // do not call delegate
                }
                else
                {
            ...
            else
            {
                ...
                let index = indexForAngle(angle: angle)
                ...
                {
                    let valsAtIndex = getSelectionDetailsAtIndex(xIndex: index)
