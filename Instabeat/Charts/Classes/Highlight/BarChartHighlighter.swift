...
public class BarChartHighlighter: ChartHighlighter
{
    public override func getHighlight(x: CGFloat, y: CGFloat) -> ChartHighlight?
    {
        if let barData = self.chart?.data as? BarChartData
        {
            ...
            if let set = barData.getDataSetByIndex(index: dataSetIndex) as? IBarChartDataSet
                , set.isStacked
            {
    ...
    public override func getSelectionDetail(xIndex: Int, y: CGFloat, dataSetIndex: Int?) -> ChartSelectionDetail?
    {
        if let barData = self.chart?.data as? BarChartData
        {
            let dataSetIndex = dataSetIndex ?? 0
            let dataSet = barData.dataSetCount > dataSetIndex ? barData.getDataSetByIndex(index: dataSetIndex) : nil
            let yValue = dataSet?.yValForXIndex(x: xIndex)
    ...
    {
        guard let entry = set.entryForXIndex(x: xIndex) as? BarChartDataEntry
            else { return nil }
