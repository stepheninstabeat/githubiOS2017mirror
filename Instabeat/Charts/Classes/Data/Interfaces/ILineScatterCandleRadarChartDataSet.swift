...
public protocol ILineScatterCandleRadarChartDataSet: IBarLineScatterCandleBubbleChartDataSet
{
    ...
    var setHigligtSelectedValueEnabled: Bool { get set}
    
    /// - returns: true if vertical highlight indicator lines are enabled (drawn)
    var isHigligtSelectedValueEnabled: Bool { get }
    
    /// - returns: true if circle highlight indicator lines are enabled (drawn)
    var isCircleHighlightIndicatorEnabled: Bool { get }
