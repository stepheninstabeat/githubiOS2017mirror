...
public class PieChartRenderer: ChartDataRendererBase
{
        ...
        for j in 0 ..< entryCount
        {
            ...
            if ((abs(e.value) > 0.000001))
            {
                if (!chart.needsHighlight(xIndex: e.xIndex,
                    dataSetIndex: data.indexOfDataSet(dataSet: dataSet)))
                {
                    ...
                    path.addRelativeArc(center: CGPoint(x: center.x, y: center.y),
                                        radius: radius, startAngle: startAngleOuter * ChartUtils.Math.FDEG2RAD, delta: sweepAngleOuter * ChartUtils.Math.FDEG2RAD)
    ...
    public override func drawHighlighted(context: CGContext, indices: [ChartHighlight])
    {
        ...
        for i in 0 ..< indices.count
        {
            ...
            guard let set = data.getDataSetByIndex(index: indices[i].dataSetIndex) as? IPieChartDataSet else { continue }
            ...
            for j in 0 ..< entryCount
            {
                guard let e = set.entryForIndex(i: j) else { continue }
                if ((abs(e.value) > 0.000001))
                {
                    visibleAngleCount += 1
                }
            ...
            context.setFillColor(set.colorAt(index: xIndex).cgColor)
            ...
            path.addRelativeArc(center: CGPoint(x: center.x, y: center.y), radius: highlightedRadius, startAngle: startAngleShifted * ChartUtils.Math.FDEG2RAD, delta: sweepAngleShifted * ChartUtils.Math.FDEG2RAD)
                ...
                (innerRadius > 0.0 || accountForSliceSpacing)
            {
                ...
                let endAngleInner = startAngleInner + sweepAngleInner
                path.addLine(to: CGPoint(x: center.x + innerRadius * cos(endAngleInner * ChartUtils.Math.FDEG2RAD), y: center.y + innerRadius * sin(endAngleInner * ChartUtils.Math.FDEG2RAD)))
                path.addRelativeArc(center: CGPoint(x: center.x, y: center.y), radius: innerRadius, startAngle: endAngleInner * ChartUtils.Math.FDEG2RAD, delta: -sweepAngleInner * ChartUtils.Math.FDEG2RAD)
            }
            else
            {
                if accountForSliceSpacing
