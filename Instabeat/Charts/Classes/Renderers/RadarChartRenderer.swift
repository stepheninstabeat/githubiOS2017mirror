...
public class RadarChartRenderer: LineRadarChartRenderer
{
    ...
    public override func drawValues(context: CGContext)
    {
        ...
        {
            let dataSet = data.getDataSetByIndex(index: i) as! IRadarChartDataSet
            ...
            for j in 0 ..< entryCount
            {
                ...
                ChartUtils.drawText(
                    ...
                    attributes: [NSFontAttributeName: valueFont,
                        NSForegroundColorAttributeName: dataSet.valueTextColorAt(index: j)]
                )
            }
        }
    }
    ...
    public override func drawHighlighted(context: CGContext, indices: [ChartHighlight])
    {
        ...
        {
            guard let set = chart.data?.getDataSetByIndex(index: indices[i].dataSetIndex) as? IRadarChartDataSet else { continue }
            ...
            if (set.isDrawHighlightCircleEnabled)
            {
                if (!_highlightPointBuffer.x.isNaN && !_highlightPointBuffer.y.isNaN)
                {
                    ...
                    if strokeColor == nil
                    {
                        strokeColor = set.colorAt(index: 0)
                    }
        ...
        strokeWidth: CGFloat)
    {
        ...
        {
            context.beginPath()
            context.addEllipse(in: CGRect(x: point.x - outerRadius,
