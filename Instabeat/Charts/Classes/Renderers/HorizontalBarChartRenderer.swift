...
public class HorizontalBarChartRenderer: BarChartRenderer
{
        ...
        for j in 0 ..< Int(ceil(CGFloat(dataSet.entryCount) * animator.phaseX))
        {
            ...
            else
            {
                ...
                for k in 0 ..< vals.count
                {
                    ...
                    if (k == 0 && !viewPortHandler.isInBoundsTop(y: barRect.origin.y + barRect.size.height))
                    {
                        // Skip to next bar
                        break
                    }
                    ...
                    if (!viewPortHandler.isInBoundsBottom(y: barRect.origin.y))
                    {
                        break
                    }
    ...
    public override func prepareBarHighlight(x: CGFloat, y1: Double, y2: Double, barspacehalf: CGFloat, trans: ChartTransformer, rect: inout CGRect)
    {
        ...
        trans.rectValueToPixelHorizontal(r: &rect, phaseY: animator?.phaseY ?? 1.0)
    }
            ...
            for dataSetIndex in 0 ..< barData.dataSetCount
            {
                ...
                if (!dataSet.isStacked)
                {
                    for j in 0 ..< Int(ceil(CGFloat(dataSet.entryCount) * animator.phaseX))
                    {
                        ...
                        if (!viewPortHandler.isInBoundsTop(y: valuePoint.y))
                        {
                            break
                        }
                        
                        if (!viewPortHandler.isInBoundsX(x: valuePoint.x))
                        {
                            continue
                        }
                        
                        if (!viewPortHandler.isInBoundsBottom(y: valuePoint.y))
                        {
                            continue
                        }
                    ...
                    for j in 0 ..< Int(ceil(CGFloat(dataSet.entryCount) * animator.phaseX))
                    {
                        ...
                        else
                        {
                            ...
                            for k in 0 ..< transformed.count
                            {
                                ...
                                if (!viewPortHandler.isInBoundsTop(y: y))
                                {
                                    break
                                }
                                
                                if (!viewPortHandler.isInBoundsX(x: x))
                                {
                                    continue
                                }
                                
                                if (!viewPortHandler.isInBoundsBottom(y: y))
                                {
                                    continue
                                }
