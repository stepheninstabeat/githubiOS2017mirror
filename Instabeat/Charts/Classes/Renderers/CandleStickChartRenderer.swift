...
public class CandleStickChartRenderer: LineScatterCandleRadarChartRenderer
{
    ...
    public func drawDataSet(context: CGContext, dataSet: ICandleChartDataSet)
    {
        ...
        {
            // get the entry
            guard let e = dataSet.entryForIndex(i: j) as? CandleChartDataEntry else { continue }
            ...
            if (showCandleBar)
            {
                ...
                trans.pointValuesToPixel(pts: &_shadowPoints)
                ...
                if (dataSet.shadowColorSameAsCandle)
                {
                    if (open > close)
                    {
                        shadowColor = dataSet.decreasingColor ?? dataSet.colorAt(index: j)
                    }
                    else if (open < close)
                    {
                        shadowColor = dataSet.increasingColor ?? dataSet.colorAt(index: j)
                    }
                    else
                    {
                        shadowColor = dataSet.neutralColor ?? dataSet.colorAt(index: j)
                    }
                }
                
                if (shadowColor === nil)
                {
                    shadowColor = dataSet.shadowColor ?? dataSet.colorAt(index: j);
                }
                ...
                trans.rectValueToPixel(r: &_bodyRect)
                
                // draw body differently for increasing and decreasing entry
                
                if (open > close)
                {
                    let color = dataSet.decreasingColor ?? dataSet.colorAt(index: j)
                ...
                {
                    let color = dataSet.increasingColor ?? dataSet.colorAt(index: j)
                ...
                {
                    let color = dataSet.neutralColor ?? dataSet.colorAt(index: j)
            ...
            else
            {
                ...
                trans.pointValuesToPixel(pts: &_rangePoints)
                trans.pointValuesToPixel(pts: &_openPoints)
                trans.pointValuesToPixel(pts: &_closePoints)
                ...
                if (open > close)
                {
                    barColor = dataSet.decreasingColor ?? dataSet.colorAt(index: j)
                }
                else if (open < close)
                {
                    barColor = dataSet.increasingColor ?? dataSet.colorAt(index: j)
                }
                else
                {
                    barColor = dataSet.neutralColor ?? dataSet.colorAt(index: j)
                }
    ...
    public override func drawValues(context: CGContext)
    {
        ...
        if (candleData.yValCount < Int(ceil(CGFloat(dataProvider.maxVisibleValueCount) * viewPortHandler.scaleX)))
        {
            ...
            for i in 0 ..< dataSets.count
            {
                ...
                for j in stride(from: minx, to: Int(ceil(CGFloat(maxx - minx) * phaseX + CGFloat(minx))), by: 1)
                {
                    guard let e = dataSet.entryForIndex(i: j) as? CandleChartDataEntry else { break }
    ...
    public override func drawHighlighted(context: CGContext, indices: [ChartHighlight])
    {
        ...
        for high in indices
        {
            ...
            {
                guard let set = candleData.getDataSetByIndex(index: dataSetIndex) as? ICandleChartDataSet else { continue }
                ...
                guard let e = set.entryForXIndex(x: xIndex) as? CandleChartDataEntry else { continue }
